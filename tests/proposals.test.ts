...
import { formatProposalNumber, normalizeProposalInput, proposalStatuses, validateProposalTransition } from "../app/lib/proposal-rules";

test("normaliza proposta em centavos e mantém os status previstos", () => {
  assert.deepEqual(proposalStatuses, ["DRAFT", "SENT", "ACCEPTED", "REJECTED", "EXPIRED"]);
  const proposal = normalizeProposalInput({ value: 250000, paymentTerms: "50% na assinatura", validityDate: "2026-09-30" });
  assert.equal(proposal.value, 250000);
  assert.equal(proposal.transportationTerms, null);
  assert.throws(() => normalizeProposalInput({ value: -1, paymentTerms: "À vista", validityDate: "2026-09-30" }), /inválido/);
  assert.throws(() => normalizeProposalInput({ value: 100, paymentTerms: "", validityDate: "2026-09-30" }), /pagamento/);
});
...
test("somente transições seguras são permitidas", () => {
  ...
  assert.throws(() => validateProposalTransition("DRAFT", "ACCEPTED"), /não permitida/);
  assert.throws(() => validateProposalTransition("ACCEPTED", "SENT"), /não permitida/);
});

test("numeração anual é legível", () => { assert.equal(formatProposalNumber(2026, 12), "PROP-2026-0012"); });

test("migration protege proposta por chaves compostas do tenant", async () => {
  const sql = await readFile(new URL("../drizzle/0008_gigantic_nitro.sql", import.meta.url), "utf8");
...
test("rotas escopam tenant, preservam versões e registram histórico", async () => {
  const collection = await readFile(new URL("../app/api/opportunities/[id]/proposals/route.ts", import.meta.url), "utf8");
  const detail = await readFile(new URL("../app/api/proposals/[id]/route.ts", import.meta.url), "utf8");
  const pdf = await readFile(new URL("../app/api/proposals/[id]/pdf/route.ts", import.meta.url), "utf8");
