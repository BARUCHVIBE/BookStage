...
test("shows possuem os estados operacionais solicitados", () => {
  ...
  assert.equal(
    validateShowTransition("CONFIRMED", "IN_PREPARATION", "PRODUCTION"),
    "IN_PREPARATION",
  );
  assert.equal(
    validateShowTransition("IN_PREPARATION", "COMPLETED", "PRODUCTION"),
    "COMPLETED",
  );
  assert.equal(
    validateShowTransition("CONFIRMED", "CANCELLED", "OWNER"),
    "CANCELLED",
  );
  assert.throws(
    () => validateShowTransition("CONFIRMED", "CANCELLED", "PRODUCTION"),
    /OWNER ou MANAGER/,
  );
  assert.throws(
    () => validateShowTransition("COMPLETED", "IN_PREPARATION", "OWNER"),
    /não permitida/,
  );
});
...
test("rider e mapa de palco aceitam apenas documentos seguros", () => {
  validateShowDocument(
    new File(["%PDF-test"], "rider.pdf", { type: "application/pdf" }),
  );
  validateShowDocument(
    new File([new Uint8Array([0x89, 0x50, 0x4e, 0x47])], "mapa.png", {
      type: "image/png",
    }),
  );
  assert.throws(
  ...
  assert.equal(
    ...
    "rider_turnê__2026_.pdf",
  );
});

test("migration protege todos os relacionamentos de Show por organização", async () => {
  const sql = await readFile(
    new URL("../drizzle/0010_fixed_radioactive_man.sql", import.meta.url),
    "utf8",
  );
...
test("arquivos técnicos ficam privados no R2 e exigem acesso ao Show", async () => {
  const route = await readFile(
    new URL("../app/api/shows/[id]/files/[kind]/route.ts", import.meta.url),
    "utf8",
  );
  const access = await readFile(
    new URL("../app/lib/show-access.ts", import.meta.url),
    "utf8",
  );
