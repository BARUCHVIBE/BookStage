...
test("proteção CSRF rejeita mutação cross-site e aceita same-origin", () => {
  const blocked = rejectCrossOriginMutation(new Request("https://bookstage.test/api/organizations", { method: "POST", headers: { origin: "https://evil.test", "sec-fetch-site": "cross-site" } }));
  assert.equal(blocked?.status, 403);
  const allowed = rejectCrossOriginMutation(new Request("https://bookstage.test/api/organizations", { method: "POST", headers: { origin: "https://bookstage.test", "sec-fetch-site": "same-origin" } }));
  assert.equal(allowed, null);
});

test("todas as rotas mutáveis aplicam proteção cross-origin", () => {
  const routes = readdirSync("app/api", { recursive: true, withFileTypes: true })
    .filter(entry => entry.isFile() && entry.name === "route.ts")
    .map(entry => join(entry.parentPath, entry.name));
  for (const route of routes) {
    const source = readFileSync(route, "utf8");
    if (/export async function (POST|PUT|PATCH|DELETE)/.test(source)) assert.match(source, /rejectCrossOriginMutation/, route);
  }
});

test("organização rejeita e-mail inválido e campos excessivos", () => {
  assert.throws(() => normalizeOrganizationInput({ name: "Empresa", email: "invalido" }));
  assert.throws(() => normalizeOrganizationInput({ name: "Empresa", email: "empresa@teste.com", logo: "x".repeat(1001) }));
  assert.equal(normalizeOrganizationInput({ name: " Empresa ", email: "contato@empresa.com" }).name, "Empresa");
});

test("oportunidade terminal não pode ser reaberta", () => {
  assert.throws(() => validateOpportunityTransition("CLOSED_WON", "NEGOTIATION"));
  assert.throws(() => validateOpportunityTransition("CLOSED_LOST", "NEW"));
  assert.equal(validateOpportunityTransition("CLOSED_WON", "CLOSED_WON"), "CLOSED_WON");
});

test("agenda limita título e normaliza observação interna", () => {
  assert.throws(() => normalizeCalendarInput({ artistId: "artist", title: "x".repeat(181), status: "OPTION", startDatetime: "2026-10-10T12:00:00Z" }));
  const input = normalizeCalendarInput({ artistId: "artist", title: "Evento", status: "OPTION", startDatetime: "2026-10-10T12:00:00Z", internalNotes: "x".repeat(5000) });
...
test("cadeia completa de migrations aplica em banco vazio", () => {
  ...
  try {
    ...
    database.exec("INSERT INTO auth_login_attempts(id,fingerprint_hash,email_hash) VALUES('x','f','e')");
    assert.equal(database.prepare("SELECT COUNT(*) AS count FROM sqlite_master WHERE type='trigger' AND name LIKE 'trg_%_status_%'").get()?.count, 8);
  } finally {
    database.close();
...
test("downloads privados aplicam sandbox e nunca expõem chave R2", () => {
  ...
  assert.match(route, /accessibleContract\(id, context\.organizationId/);
