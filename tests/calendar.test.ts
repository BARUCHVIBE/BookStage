...
import { calendarStatuses, canManageCalendar, canViewCalendar, intervalsOverlap, isBlockingStatus, normalizeCalendarInput } from "../app/lib/calendar-rules";

test("cria dados válidos para cada status da agenda", () => {
  for (const status of calendarStatuses) {
    const input = normalizeCalendarInput({ artistId:"artist-a", title:`Evento ${status}`, status, startDatetime:"2026-09-12T18:00:00.000Z", endDatetime:"2026-09-12T20:00:00.000Z" });
...
test("rejeita término anterior ao início", () => {
  assert.throws(() => normalizeCalendarInput({ artistId:"artist-a", title:"Evento", status:"OPTION", startDatetime:"2026-09-12T20:00:00.000Z", endDatetime:"2026-09-12T18:00:00.000Z" }), /término/);
});

test("detecta sobreposição e limita bloqueio a confirmado ou bloqueado", () => {
  assert.equal(intervalsOverlap("2026-09-12T18:00:00.000Z","2026-09-12T20:00:00.000Z","2026-09-12T19:00:00.000Z",null),true);
  assert.equal(intervalsOverlap("2026-09-12T18:00:00.000Z","2026-09-12T20:00:00.000Z","2026-09-13T19:00:00.000Z",null),false);
...
test("migration garante isolamento e conflitos no banco", async () => {
  const sql = await readFile(new URL("../drizzle/0003_black_deathbird.sql", import.meta.url),"utf8");
...
test("APIs mantêm filtros, edição e exclusão escopados pela organização", async () => {
  const collection = await readFile(new URL("../app/api/calendar/route.ts",import.meta.url),"utf8");
  const item = await readFile(new URL("../app/api/calendar/[id]/route.ts",import.meta.url),"utf8");
  ...
  assert.match(item,/DELETE FROM calendar_entries WHERE id=\? AND organization_id=\?/);
