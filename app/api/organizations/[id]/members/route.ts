...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const member = await env.DB.prepare(`SELECT user_id AS userId, organization_id AS organizationId, role, status FROM memberships WHERE user_id=? AND organization_id=?`).bind(user.id,id).first<{userId:string;organizationId:string;role:string;status:string}>();
  try { assertTenantAccess(user.id, member, id); }
  catch (e) { return Response.json({ error: (e as TenantAccessError).message }, { status: 404 }); }
  if (!member || !["OWNER","MANAGER"].includes(member.role)) return Response.json({ error: "Sem permissão para visualizar a equipe." }, { status: 403 });
