...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!b) return Response.json({ error: "Requisição inválida." }, { status: 400 });
  let input;try{input=normalizeOrganizationInput(b)}catch(error){return Response.json({error:error instanceof Error?error.message:"Dados inválidos."},{status:400})}
  await env.DB.prepare(`UPDATE organizations SET name=?,logo=?,email=?,phone=?,document=?,website=?,instagram=?,description=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(input.name,input.logo,input.email,input.phone,input.document,input.website,input.instagram,input.description,id).run();
