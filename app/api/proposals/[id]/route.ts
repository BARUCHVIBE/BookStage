...
import { normalizeProposalInput, validateProposalTransition, type ProposalStatus } from "@/app/lib/proposal-rules";
import { rejectCrossOriginMutation } from "@/app/lib/request-security";

const statusActivity: Record<Exclude<ProposalStatus, "DRAFT">, { type: string; description: string }> = {
...
export async function PATCH(request: Request, route: { params: Promise<{ id: string }> }) {
  ...
  if ("status" in body) {
    let status;
    try { status = validateProposalTransition(access.status, body.status); }
    ...
    const statements = [
      env.DB.prepare(`UPDATE proposals SET status=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=?`).bind(status, id, context.organizationId),
      env.DB.prepare(`INSERT INTO opportunity_activities (id,organization_id,opportunity_id,type,description,from_value,to_value,created_by) VALUES (?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(), context.organizationId, access.opportunityId, activity.type, `${activity.description} ${access.proposalNumber}.`, access.status, status, context.user.id),
    ];
    if (status === "SENT") statements.push(env.DB.prepare(`UPDATE opportunities SET stage=CASE WHEN stage IN ('NEW','CONTACTED','QUALIFIED') THEN 'PROPOSAL' ELSE stage END,updated_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=?`).bind(access.opportunityId, context.organizationId));
    await env.DB.batch(statements);
    return Response.json({ ok: true, status });
  }
  if (access.status !== "DRAFT") return Response.json({ error: "Somente propostas em rascunho podem ser editadas." }, { status: 409 });
  let input;
  try { input = normalizeProposalInput(body); }
  catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Dados inválidos." }, { status: 400 }); }
  await env.DB.prepare(`UPDATE proposals SET value=?,payment_terms=?,transportation_terms=?,accommodation_terms=?,technical_terms=?,additional_terms=?,validity_date=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=? AND status='DRAFT'`).bind(input.value, input.paymentTerms, input.transportationTerms, input.accommodationTerms, input.technicalTerms, input.additionalTerms, input.validityDate, id, context.organizationId).run();
