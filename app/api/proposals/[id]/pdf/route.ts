...
export async function GET(
  ...
  const line = (
    ...
    const size = options.size ?? 10,
      ...
      lines = wrap(text, font, size, contentWidth);
    ...
    for (const item of lines) {
      page.drawText(item, {
        ...
        size,
        font,
        color: options.color ?? rgb(0.17, 0.2, 0.22),
      });
      y -= size + 4;
    }
    y -= options.gap ?? 2;
  ...
  const section = (title: string, content: string | null) => {
    ...
    line(title.toUpperCase(), {
      size: 8,
      font: bold,
      color: rgb(0.63, 0.43, 0.1),
      gap: 7,
    });
    line(content, { size: 10, gap: 14 });
  };
  ...
  page.drawRectangle({
    ...
    width: 44,
    height: 4,
    color: rgb(0.79, 0.58, 0.2),
  });
  page.drawText(proposal.organizationName, {
    x: margin,
    y: pageHeight - 72,
    size: 18,
    font: bold,
  ...
  page.drawText(proposal.proposalNumber, {
    x: margin,
    y: pageHeight - 128,
    size: 13,
    font: bold,
  ...
  y = pageHeight - 190;
  line(`Apresentamos a proposta para contratação de ${proposal.artistName}.`, {
    size: 16,
    font: bold,
    gap: 18,
  });
  section(
    "Contratante",
    `${proposal.customerName}${proposal.companyName ? ` - ${proposal.companyName}` : ""}\n${proposal.customerEmail} | ${proposal.customerPhone}`,
  );
  section(
    "Evento",
  ...
  page.drawText("VALOR DA PROPOSTA", {
    x: margin + 18,
    y: y - 8,
    size: 8,
    font: bold,
    color: rgb(0.44, 0.35, 0.2),
  });
  page.drawText(money(proposal.value), {
    x: margin + 18,
    y: y - 34,
    size: 20,
    font: bold,
  ...
  section(
    "Validade",
    `Esta proposta é válida até ${date(proposal.validityDate)}.`,
  );
  line(
    `Contato: ${proposal.organizationEmail}${proposal.organizationPhone ? ` | ${proposal.organizationPhone}` : ""}${proposal.organizationWebsite ? ` | ${proposal.organizationWebsite}` : ""}`,
    { size: 8, color: rgb(0.38, 0.4, 0.42) },
  );
  ...
  return new Response(bytes as BodyInit, {
    headers: {
      ...
      "content-disposition": `attachment; filename="${proposal.proposalNumber.toLowerCase()}.pdf"`,
