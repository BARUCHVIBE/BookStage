...
export async function PATCH(
  ...
  if (!canManageArtistAssignments(context.membership.role))
    return Response.json(
      { error: "Sem permissão para editar o catálogo." },
      { status: 403 },
    );
  const { id } = await routeContext.params;
  ...
  for (const [key, max] of Object.entries(limits))
    if (key in body && typeof body[key] !== "string")
      ...
      return Response.json(
        { error: `Campo ${key} excede o limite permitido.` },
        { status: 400 },
      );
  if ("isPublic" in body && typeof body.isPublic !== "boolean")
    return Response.json(
      { error: "Visibilidade pública inválida." },
      { status: 400 },
    );
  ...
  const requestedSlug =
    makeSlug(
      "slug" in body ? (text("slug") ?? name) : (existing.slug ?? name),
    ) || `artista-${id.slice(0, 6)}`;
  const duplicate = await env.DB.prepare(
    `SELECT id FROM artists WHERE organization_id=? AND slug=? AND id<>?`,
  )
    .bind(context.organizationId, requestedSlug, id)
    .first();
  if (duplicate)
    return Response.json(
  ...
  const mapping = [
    ...
    ["baseCity", "base_city"],
    ["showFormats", "show_formats"],
    ["videoUrls", "video_urls"],
    ["instagram", "instagram"],
    ["spotify", "spotify"],
    ["youtube", "youtube"],
    ["publicMaterials", "public_materials"],
  ] as const;
