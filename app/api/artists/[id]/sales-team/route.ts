...
export async function PUT(
  ...
  let assignments;
  try {
    assignments = validateCommercialAssignments(
      context.organizationId,
      body.primaryUserId || null,
      Array.isArray(body.authorizedUserIds) ? body.authorizedUserIds : [],
      membershipRows.results as AssignmentInput[],
    );
  } catch (error) {
    return Response.json(
      {
        error: error instanceof Error ? error.message : "Atribuição inválida.",
      },
      { status: 400 },
    );
  }
  ...
  if (assignments.primaryUserId)
    statements.push(
      env.DB.prepare(
        `INSERT INTO artist_sales_assignments (organization_id,artist_id,user_id,is_primary) VALUES (?,?,?,1)`,
      ).bind(context.organizationId, id, assignments.primaryUserId),
    );
  for (const userId of assignments.authorizedUserIds)
    statements.push(
      env.DB.prepare(
