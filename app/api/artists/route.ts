...
export async function GET(request: Request) {
  ...
  const responsibleScope = responsibleId
    ? `AND EXISTS (SELECT 1 FROM artist_sales_assignments filter_assignment WHERE filter_assignment.artist_id=a.id AND filter_assignment.organization_id=a.organization_id AND filter_assignment.user_id=? AND filter_assignment.is_primary=1)`
    : "";
...
export async function POST(request: Request) {
  ...
  await env.DB.prepare(
    ...
    .run();
  return Response.json(
    { artist: { id, name, slug, status: "ACTIVE" } },
    { status: 201 },
  );
}
