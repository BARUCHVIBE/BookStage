...
function urls(value: string | null) {
  return lines(value)
...
export async function getPublicOrganization(slug: string) {
  const row = await env.DB.prepare(
    `SELECT slug,name,logo,description,instagram,website FROM organizations WHERE slug=? AND status='ACTIVE'`,
  )
    .bind(slug)
...
export async function getPublicArtists(organizationSlug: string) {
  const rows = await env.DB.prepare(
    `SELECT artist.slug,artist.name,artist.photo_url AS photoUrl,artist.cover_url AS coverUrl,artist.genre,artist.base_city AS baseCity FROM artists artist JOIN organizations organization ON organization.id=artist.organization_id WHERE organization.slug=? AND organization.status='ACTIVE' AND artist.status='ACTIVE' AND artist.is_public=1 AND artist.slug IS NOT NULL ORDER BY artist.name`,
  )
...
export async function getPublicArtist(
  ...
  const row = await env.DB.prepare(
    `SELECT artist.slug,artist.name,artist.photo_url AS photoUrl,artist.cover_url AS coverUrl,artist.genre,artist.description,artist.base_city AS baseCity,artist.show_formats AS showFormats,artist.video_urls AS videoUrls,artist.instagram,artist.spotify,artist.youtube,artist.public_materials AS publicMaterials FROM artists artist JOIN organizations organization ON organization.id=artist.organization_id WHERE organization.slug=? AND artist.slug=? AND organization.status='ACTIVE' AND artist.status='ACTIVE' AND artist.is_public=1`,
  )
    .bind(organizationSlug, artistSlug)
    .first<
      Omit<PublicArtist, "showFormats" | "videos" | "materials"> & {
  ...
  return {
    ...
    showFormats: lines(showFormats),
    videos: urls(videoUrls),
    materials: urls(publicMaterials),
  } as PublicArtist;
}
