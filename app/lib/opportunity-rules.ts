...
export function validateOpportunityTransition(
  ...
  if (
    (current === "CLOSED_WON" || current === "CLOSED_LOST") &&
    next !== current
  )
    throw new Error("Oportunidades encerradas não podem retornar ao pipeline.");
  return next;
}
...
export function validateStageChange(
  ...
  const reason =
    typeof lostReason === "string" ? lostReason.trim().slice(0, 500) : "";
...
export function parseProposedValue(value: unknown) {
  ...
  if (!Number.isInteger(parsed) || parsed < 0 || parsed > 999_999_999_99)
