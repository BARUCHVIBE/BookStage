...
export async function accessibleShow(
  ...
  const show = await env.DB.prepare(
    `SELECT show.id,show.organization_id AS organizationId,show.opportunity_id AS opportunityId,show.calendar_entry_id AS calendarEntryId,show.date,show.fee,show.status,show.rider_file_key AS riderFileKey,show.rider_file_name AS riderFileName,show.rider_file_type AS riderFileType,show.rider_file_size AS riderFileSize,show.stage_map_file_key AS stageMapFileKey,show.stage_map_file_name AS stageMapFileName,show.stage_map_file_type AS stageMapFileType,show.stage_map_file_size AS stageMapFileSize,opportunity.assigned_user_id AS assignedUserId,opportunity.originator_user_id AS originatorUserId,opportunity.commercial_validator_user_id AS commercialValidatorUserId FROM shows show JOIN opportunities opportunity ON opportunity.id=show.opportunity_id AND opportunity.organization_id=show.organization_id WHERE show.id=? AND show.organization_id=?`,
  )
  ...
  return show &&
    canViewShow(
      ...
      show.assignedUserId,
      userId,
      show.originatorUserId,
      show.commercialValidatorUserId,
    )
    ? show
    : null;
}
